...
def conv_net(x):
    ...
    # Convolution Layer 2
    conv2 = tf.layers.conv2d(conv1, filters=64, kernel_size=[5, 5], padding='same', activation=tf.nn.relu, name='conv2')
...
with tf.Session() as sess:
    ...
    # Save weights
    with open("conv1_weights.param", "wb") as f:
        pickle.dump(sess.run(conv1_weights), f)
    with open("conv2_weights.param", "wb") as f:
        pickle.dump(sess.run(conv2_weights), f)
    with open("fc1_weights.param", "wb") as f:
        pickle.dump(sess.run(fc1_weights), f)
    with open("out_weights.param", "wb") as f:
    ...
    with open("conv1_biases.param", "wb") as f:
        pickle.dump(sess.run(conv1_biases), f)
    with open("conv2_biases.param", "wb") as f:
        pickle.dump(sess.run(conv2_biases), f)
    with open("fc1_biases.param", "wb") as f:
        pickle.dump(sess.run(fc1_biases), f)
    with open("out_biases.param", "wb") as f:
